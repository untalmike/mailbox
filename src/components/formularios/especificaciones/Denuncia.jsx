...
const Denuncia = ({respond_form, setRespond_form, botones}) => {
    ...
    return ( 
        <div className={(accion === "Denuncia" && anonimato === "Sí" && botones === 5) || (accion === "Denuncia" && anonimato === "No" && botones === 6) ? "" : "hidden"}>
            ...
            <div className="">
                <span className='text-xl'>Agradecemos tu confianza, por favor responde las siguientes preguntas para realizar un seguimiento y dar solución a tu denuncia.</span>
                <div className="grid space-y-5 space-x-2 p-2 sm:grid-cols-1 lg:grid-cols-1 gap-3">
                    <div className='grid grid-rows-1'>
                        ...
                        <input type="date" className='rounded-xl text-xl border-b-gray-300 mt-6 outline-none p-4 shadow-xl' onChange={(event) => setRespond_form({...respond_form, [event.target.name] : event.target.value})} name="fechaDenuncia"/>
                    </div>
                    <div className='grid grid-rows-1'>
                        <span className='text-xl'>Indícanos cual fue la hora en la que sucedieron los hechos </span>
                        <input type="time" className='rounded-xl text-xl border-b-gray-300 mt-6 outline-none p-4 shadow-xl' onChange={(event) => setRespond_form({...respond_form, [event.target.name] : event.target.value})} name="horaDenuncia"/>
                    </div>
                    <div className='grid grid-rows-1'>
                        <span className='text-xl'>En qué área del call center sucedieron los hechos </span>
                        <input type="text" className='rounded-xl text-xl border-b-gray-300 mt-6 outline-none p-4 shadow-xl' onChange={(event) => setRespond_form({...respond_form, [event.target.name] : event.target.value})} name="lugarDenuncia"/>
                    </div>
                    <div className='grid grid-rows-1'>
                        <span className='text-xl'>¿Como te enteraste de los hechos?</span>
                        <select className='rounded-xl text-xl border-b-gray-300 mt-6 outline-none p-4 shadow-xl' onChange={(event) => setRespond_form({...respond_form, [event.target.name] : event.target.value})} name="descripcionDenuncia">
